...
OpenPairs <- read_sf("OpenPairs_ForestDry.shp") %>% 
  st_transform(crs = "+proj=longlat +datum=WGS84 +no_defs") %>% mutate(NewLabel = paste(substr(ID_GPX,8,9),Group, sep = "_"))
```
...
l <- l %>% 
 ...
 addPolylines(data = as_Spatial(Outline), color = "blue", fillOpacity = 0, group = "Outline") %>% 
  addLegend("bottomleft", pal = pal ,title = "Kattrup sites",opacity = 1, values = as.character(sort(unique(Points$Type))),  group = "Legend old") %>%
  addLegend("bottomleft", pal = NewPal ,title = "Kattrup new open pairs",opacity = 1, values = as.character(sort(unique(OpenPairs$NewLabel))),  group = "Legend New") %>%
