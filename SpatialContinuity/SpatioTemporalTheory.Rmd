...
```{r quadSol, echo = F, include=FALSE}
Solution_quad <- suppressMessages(solution_quadratic(layers_habitat = Phillips, layer_cost = PhilCost, Dist = 111000, nchains = 2))

names(Solution_quad$FinalStack) <- c("T0", "T1")

Solution_quad_DF <- Solution_quad$FinalStack %>%
  as("SpatialPixelsDataFrame") %>%
  as.data.frame() %>% pivot_longer(starts_with("T"), names_to = "Time", values_to = "Flow") 

G <- ggplot() + geom_sf(data = PhillipsSF, size = 0.1) + geom_sf_label(data = PhillipsSF, aes(label = ID)) + geom_tile(data = Solution_quad_DF, aes(x = x, y = y, fill = Flow), alpha = 0.6) + geom_sf_label(data = PhillipsSF, aes(label = ID)) + facet_wrap(~Time) + theme_bw() + theme(panel.spacing = unit(6, "lines")) + ylab("") + xlab("")  + theme(axis.text.x = element_blank(), axis.text.y = element_blank(),axis.ticks = element_blank()) + scale_fill_gradient2(high = muted("red"), mid = "#ffffbf",low = muted("blue"), midpoint =0.5, na.value = "transparent", name = "Flow")
```

```{r solG, echo = F, cache=T}
print(G)
```

## Graph solution

```{r graphQuadSols, echo = F}
a <- suppressMessages(Problem_To_Graph(habitat_layers = Phillips, cost_layer = PhilCost, Dist = 111000))

b <- a$graph %>% activate(edges) %>% as.data.frame() %>% bind_cols(data.frame(flow = c(1,0.45,0.55,0.333,0.333,0.333,0.45,0.55,0.333,0.333,0.333,1)))
a$graph <- suppressMessages(a$graph %>% activate(edges) %>% full_join(b))


ggraph(a$graph, a$coords) + geom_edge_link(aes(width = flow))+  geom_node_point(size = 8, colour = 'steelblue') + geom_node_text(aes(label = Node_Name)) + scale_y_reverse() + theme_void() + scale_edge_width(limits = c(0.3,1), range = c(0,2))
```
