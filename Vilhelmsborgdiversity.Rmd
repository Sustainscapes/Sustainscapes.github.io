...
MaxHabitatRast[MaxHabitatRast == 0] <- NA
MaxHabitatRastHab <- terra::rast("MaxHabitatRastHab.tif")
MaxHabitatRastHab[MaxHabitatRastHab == 0] <- NA
```
  ...
  leaflet::addLegend(pal = palFac, 
            ...
            labFormat  = labelFormat(
              transform = function(x) {
                levels(MaxHabitatRastHab)[[1]]$ForestDryPoor[which(levels(MaxHabitatRastHab)[[1]]$value == x)]
              }), group = "Best habitat") |> 
...
in this next map we used the maximum value for Forest and Open and see what is the highest value of Richness for each

```{r}
Test <- terra::as.data.frame(Habs_VilhelmsBorg,cells = T) |> 
  tidyr::pivot_longer(-cell, names_to = "Habitat", values_to = "Richness") |>
  mutate(Richness = ifelse(is.na(Richness), 0, Richness)) |> 
  mutate(Habitat  = case_when(stringr::str_detect(Habitat, "Forest") ~ "Forests",
                              stringr::str_detect(Habitat, "Open") ~ "Open")) |> group_split(Habitat) |>
  purrr::map(~group_by(.x, cell)) |> 
  purrr::map(~dplyr::slice_max(.x, order_by =Richness, n = 1, with_ties = F))
...
for(i in 1:length(Test)){
  SubHab[[i]] <- Temp
  values(SubHab[[i]])[Test[[i]]$cell] <- Test[[i]]$Richness
}

SubHab <- SubHab |> purrr::reduce(c)
names(SubHab) <- Names
```

```{r}
pal <- colorNumeric(c("#3288bd", "#99d594", "#e6f598", "#ffffbf", "#fee08b", "#fc8d59", 
                               "#d53e4f"), values(SubHab),
                    na.color = "grey")
...
for(i in 1:nlyr(SubHab)){
  l <- l |> addRasterImage(SubHab[[i]], colors = pal, opacity = 0.7, group = names(SubHab[[i]]))
}
  
  
l |> 
  leaflet::addLegend(pal = pal, values = values(SubHab), title = "Species richness") |> 
  addLayersControl(
    baseGroups = names(SubHab),
    options = layersControlOptions(collapsed = T)
  )
```
